        ...
        <div className="grid md:grid-cols-2 gap-10 items-start">
          ...
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border bg-card px-3 py-1 text-xs text-muted-foreground">
              <NotebookPen className="h-3.5 w-3.5" /> Notes Application
            </div>
            ...
            <ul className="mt-4 space-y-2 text-sm grid grid-cols-2 gap-1 sm:gap-2">
              {notesFeatures.map((feature, index) => (
                <li key={index} className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-5 w-5 text-primary" /> {feature.title}</li>
              ))}
            </ul>
